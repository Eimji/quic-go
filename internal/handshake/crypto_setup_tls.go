...
handshakeLoop:
	for {
		alert := h.tls.Handshake()
		if alert != mint.AlertNoAlert {
			return fmt.Errorf("TLS handshake error: %s (Alert %d)", alert.String(), alert)
		}
		if h.tls.State() == mint.StateClientStart { // this happens if a stateless retry is performed
			return ErrCloseSessionForRetry
		}
		if h.tls.State() == mint.StateClientConnected || h.tls.State() == mint.StateServerConnected {
			break handshakeLoop
		}
	}
