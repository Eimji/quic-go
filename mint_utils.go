...
func packUnencryptedPacket(aead crypto.AEAD, hdr *wire.Header, sf *wire.StreamFrame, pers protocol.Perspective) ([]byte, error) {
	...
	// pad the ClientHello to the required packet size
	var addPadding bool
	if hdr.Type == protocol.PacketTypeInitial {
		// padding gets added to the end of the packet. Make sure the STREAM_FRAME has the data len set
	...
	if addPadding {
		...
		buffer.Write(bytes.Repeat([]byte{0}, 1200-buffer.Len()-aead.Overhead()))
	}
